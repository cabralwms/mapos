...
class Compras extends MY_Controller
{
    ...
    public function gerenciar()
    {
        ...
        if (count($where_array) > 0) {
            ...
            $this->data['configuration']['first_url'] = base_url("index.php/compras/gerenciar")."\?pesquisa={$pesquisa}&status={$status}&data={$de}&data2={$ate}";
        }

        $this->pagination->initialize($this->data['configuration']);

        $this->data['results'] = $this->compras_model->get('compras', '*', $where_array, $this->data['configuration']['per_page'], $this->uri->segment(3));
    ...
    if ($this->form_validation->run('compras') == false) {
        ...
        try {
            $dataCompra = DateTime::createFromFormat('d/m/Y', $dataCompra)->format('Y-m-d');
        } catch (Exception $e) {
        ...
        if (is_numeric($id = $this->compras_model->add('compras', $data, true))) {
            ...
            redirect(site_url('compras/editar/') . $id); // Redirecionar para a página de edição da compra
        } else {
    ...
    public function adicionarProduto()
{
    ...
    // Verifica se a compra é editável
    ...
    if ($this->form_validation->run() == false) {
        ...
        if ($this->compras_model->add('itens_de_compras', $data) == true) {
            ...
            // Altera o valor de precoCompra
            $this->db->set('precoCompra', $preco);
    ...
    public function atualizar_estoque_produto() {
        // Obter os dados do POST
        $idProduto = $this->input->post('id');
        $novaQuantidade = $this->input->post('quantidade');

        // Chamar a função do model para atualizar o estoque
        $resultado = $this->Compras_model->atualizar_estoque_produto($idProduto, $novaQuantidade);

        if ($resultado) {
            // Se a atualização for bem-sucedida, enviar uma resposta JSON de sucesso
            $this->output->set_output(json_encode(['status' => 'success']));
        } else {
            // Se houver um erro, enviar uma resposta JSON de erro
