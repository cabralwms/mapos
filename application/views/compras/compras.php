...
<div class="new122">
  ...
  <div class="span12" style="margin-left: 0">
                <form method="get" action="<?php echo base_url(); ?>index.php/compras/gerenciar">
                <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'aCompra')) { ?>
            <div class="span3">
                <a href="<?php echo base_url(); ?>index.php/compras/adicionar" class="button btn btn-mini btn-success" style="max-width: 160px">
                    ...
                    <?php
                        ...
                        foreach ($results as $r) {
                            ...
                            if ($r->garantia && is_numeric($r->garantia)) {
                                $vencGarantia = dateInterval($r->dataCompra, $r->garantia);
                            }
                                $corGarantia = '';
                            ...
                            if ($this->permission->checkPermission($this->session->userdata('permissao'), 'vCompra')) {
                                echo '<a style="margin-right: 1%" href="' . base_url() . 'index.php/compras/visualizar/' . $r->idCompras . '" class="btn-nwe" title="Ver mais detalhes"><i class="bx bx-show bx-xs"></i></a>';
                                echo '<a style="margin-right: 1%" href="' . base_url() . 'index.php/compras/imprimir/' . $r->idCompras . '" target="_blank" class="btn-nwe6" title="Imprimir A4"><i class="bx bx-printer bx-xs"></i></a>';
                                echo '<a style="margin-right: 1%" href="' . base_url() . 'index.php/compras/imprimirTermica/' . $r->idCompras . '" target="_blank" class="btn-nwe6" title="Imprimir Não Fiscal"><i class="bx bx-printer bx-xs"></i></a>';
                            }
                            if ($this->permission->checkPermission($this->session->userdata('permissao'), 'eCompra')) {
                                echo '<a style="margin-right: 1%" href="' . base_url() . 'index.php/compras/editar/' . $r->idCompras . '" class="btn-nwe3" title="Editar venda"><i class="bx bx-edit bx-xs"></i></a>';
                            }
                            // if ($this->permission->checkPermission($this->session->userdata('permissao'), 'eCompra')) {
                             // echo '<a href="#modal-excluir" role="button" data-toggle="modal" venda="' . $r->idCompras
                                echo '</td>';
                                echo '</tr>';
                        } ?>
                </tbody>
